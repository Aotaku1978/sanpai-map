...
export default function Home() {
  ...
  const filtered = useMemo(() => facilities.filter((f) => {
    ...
    if (typeFilter === 'sanpai' && f.license_type?.includes('特別管理')) return false
    if (typeFilter === 'tokubetsu' && !f.license_type?.includes('特別管理')) return false
    return true
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        <div style={{ flex: 1 }}>
          <FacilityMap facilities={filtered} onSelect={setSelected} />
        </div>
        <div style={{ width: 260, borderLeft: '1px solid #e5e7eb', overflowY: 'auto', background: '#fff' }}>
          {selected ? (
            <div style={{ padding: 14 }}>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 8 }}>{selected.name}</div>
              <div style={{ fontSize: 12, color: '#374151', lineHeight: 1.8 }}>
                <div>📍 {selected.address}</div>
                <div>🔢 {selected.license_no}</div>
                <div>📋 {selected.license_type}</div>
                <div>📅 期限：{selected.expire_date}</div>
          ...
          <div style={{ borderTop: '1px solid #e5e7eb' }}>
            {filtered.map(f => (
              <div key={f.id} onClick={() => setSelected(f)} style={{ padding: '8px 14px', borderBottom: '1px solid #f3f4f6', cursor: 'pointer', background: selected?.id === f.id ? '#f0fdf4' : '#fff', fontSize: 12 }}>
                <div style={{ fontWeight: 500 }}>{f.name}</div>
                <div style={{ color: '#6b7280', fontSize: 11 }}>{f.address?.replace(/東京都|埼玉県|神奈川県|宮城県|福岡県|大阪府/, '')}</div>
              </div>
            ))}
          </div>
        </div>
